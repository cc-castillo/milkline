...
export default function BuyerInbox() {
  ...
  const messageBgBuyer = useColorModeValue('cyan.100', 'cyan.800');
  ...
  useEffect(() => {
    const loadInbox = async () => {
      try {
        ...
        const buyerResponse = await fetch('/api/user-profile', {
          method: 'GET',
          headers: {
        ...
        if (buyerResponse.ok) {
          ...
          if (buyerData.profile?.id) {
            setCurrentBuyerId(buyerData.profile.id);
          }
        }
  ...
  const handleSendReply = async () => {
    ...
    try {
      ...
      const buyerData = await buyerResponse.json();
      const buyerId = buyerData.profile?.id;
  ...
  const handleGenerateAIResponse = async () => {
    if (!selectedThread || !currentBuyerId) {
      toast({
        title: 'Error',
    ...
    try {
      ...
      const response = await fetch('/api/messages/ai-generate', {
        ...
        body: JSON.stringify({
          ...
          sellerName: selectedThread.sellerName,
                    ...
                    <CardHeader>
                      <VStack align="start" spacing={2}>
                        ...
                        <Text fontSize="sm" color={textColor}>
                          {selectedThread.sellerName}{selectedThread.rfqId ? ` • RFQ ${selectedThread.rfqId.slice(0, 8)}` : ''}
                        </Text>
                      </VStack>
                    </CardHeader>
                    <CardBody>
                      <VStack spacing={4} align="stretch" maxH="500px" overflowY="auto">
                        {selectedThread.messages.map((message) => {
                          ...
                          // senderId is a string, currentBuyerId is a number
                          const isFromCurrentUser = currentBuyerId && message.senderId && Number(message.senderId) === currentBuyerId;
                          ...
                          return (
                            ...
                            >
                              <Box
                                bg={isFromCurrentUser ? messageBgBuyer : messageBgSeller}
                                color={isFromCurrentUser ? messageTextBuyer : messageTextSeller}
                                p={3}
                                borderRadius="md"
                              >
